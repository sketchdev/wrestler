...
exports.validateEmailUniqueness = async (req, email, errors, key = 'email') => {
  if (email && errors[key] === undefined && (await req.wrestler.dbDriver.countDocuments(req.wrestler.resource, { email })) !== 0) {
    errors[key] = { messages: ['Email already exists'] };
  }
};
