...
const parseBody = async (req) => {
  this.newEmail = req.body.newEmail;
...
const validateRequest = async (req, res) => {
  const errors = {};
  if (!this.newEmail) {
    errors.newEmail = { messages: ['Email is required'] };
  }
  if (this.newEmail && !validator.isEmail(this.newEmail)) {
    errors.newEmail = { messages: ['Email is invalid'] };
  }
  await validateEmailUniqueness(req, this.newEmail, errors, 'newEmail');
...
const updateUser = async (req) => {
  ...
  await req.wrestler.dbDriver.findOneAndUpdate(req.wrestler.resource, { email }, { changeEmailCode, changeEmailExpiresAt, newEmail });
...
const sendEmail = async (req, res) => {
  const { email } = req.wrestler.user;
  const { changeEmailCode, newEmail } = this;
