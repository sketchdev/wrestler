...
exports.handleRestfulPutRequest = async (req, res, next) => {
  if (req.method === 'PUT') {
    ...
    const filter = appendFilter(req, { _id: req.wrestler.dbDriver.toObjectId(req.wrestler.id) });
...
exports.handleRestfulPatchRequest = async (req, res, next) => {
  if (req.method === 'PATCH') {
    ...
    const filter = appendFilter({ _id: req.wrestler.dbDriver.toObjectId(req.wrestler.id) });
