...
const { appendFilter, appendUpdatedBy, patch } = require('./common');
...
exports.handleRestfulGetRequest = async (req, res, next) => {
  try {
    if (req.method === 'GET') {
      ...
      if (req.wrestler.id) {
        const filter = appendFilter(req.wrestler.user, { id: req.wrestler.id });
        ...
        const filter = appendFilter(req.wrestler.user, req.query);
        const docs = await req.wrestler.dbDriver.findMany(req.wrestler.resource, filter, projection, { sort, limit: limit + 1, skip });
...
exports.handleRestfulPutRequest = async (req, res, next) => {
  try {
    if (req.method === 'PUT') {
      ...
      const now = new Date();
      const doc = appendUpdatedBy(req.wrestler.user, Object.assign({}, req.body, { createdAt: now, updatedAt: now }));
      const filter = appendFilter(req.wrestler.user, { id: req.wrestler.id });
...
exports.handleRestfulPatchRequest = async (req, res, next) => {
  try {
    if (req.method === 'PATCH') {
      ...
      res.json(await patch(req.wrestler)(req.wrestler.id, req.body));
    }
    next();
  } catch (err) {
