...
exports.handleRestfulPutRequest = async (req, res, next) => {
  if (req.method === 'PUT') {
    ...
    const now = new Date();
    const doc = appendUserId(req, Object.assign({}, req.body, { createdAt: now, updatedAt: now }));
    const filter = { _id: req.wrestler.dbDriver.toObjectId(req.wrestler.id) };
    const replacedDoc = await req.wrestler.dbDriver.findOneAndReplace(req.wrestler.resource, filter, doc);
    const result = transformOneId(replacedDoc);
    res.json(result);
  }
  next();
};
...
const appendUserId = (req, doc) => {
  // TODO: eliminate the dependency of `common` here
  if (req.wrestler && req.wrestler.user && req.wrestler.user.id && req.wrestler.resource !== common.USER_COLLECTION_NAME) {
