...
exports.checkAuthentication = async (req, res, next) => {
  if (req.wrestler.options.users && requiresAuthentication(req)) {
  ...
  if (!res.headersSent) {
    ...
    if (allows.length > 0) {
      ...
      for (const allow of allows) {
        const resourceAuthorized = allow.resource === req.resource;
        const roleAuthorized = allow.roles.includes(req.session.user.role);
