    ...
    if (req.headers.authorization) {
      ...
      if (scheme === 'Bearer') {
        try {
          ...
          res.status(401).json({ base: { messages: ['Invalid authorization token'] }});
        }
      }
    }
 ...
 *
 * if users = true then
 *  scope everything to the user logged in
 *  allow any user to CRUD themselves
 * if users.authorization = 'roles' then
 *  create a root user (use ROOT_USER/ROOT_PASS if found) with role 'admin'
 *  allow 'admin' to CRUD any user (including setting the role value)
 *  allow any user to CRUD themselves; however, they can only set their role to 'guest'
 * if users.authorization = function then
 *  call this function(req, res, next)
 */
