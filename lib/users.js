...
exports.handleLogin = async (req, res, next) => {
  try {
    if (isUserRequest(req) && isLogin(req)) {
      let user = await req.wrestler.dbDriver.findOne(req.wrestler.resource, { email: req.body.email });
      ...
      try {
        const token = await generateJwt(req.wrestler, user);
...
exports.handleUserGetRequest = async (req, res, next) => {
  try {
    if (isUserRequest(req) && req.method === 'GET') {
      const customTransform = res.wrestler.transformer || ((x) => x);
      res.wrestler.transformer = req.wrestler.id ? (x) => customTransform(transformOne(x)) : (x) => customTransform(transformMany(x));
    }
    next();
  } catch (err) {
    next(err);
...
/**
 * For tokens that are near expiration, calling this method will return a new JWT with an extended expiration date.
 *
 * @param {*} req  Inbound request object
 * @param {*} res  Outbound response object
 * @param {*} next Next piece of middleware in the chain to handle the {@code req}
 */
exports.handleUserRefreshTokenRequest = async (req, res, next) => {
  try {
    if (isUserRequest(req) && isRefreshToken(req)) {
      try {
        const user = await req.wrestler.dbDriver.findOne(req.wrestler.resource, { id: req.wrestler.user.id });
...
const buildConfirmation = () => {
  return buildEmailCodeWithExpiry('confirmation', 1, 'day');
};

const buildInvite = () => {
  return buildEmailCodeWithExpiry('invite', 30, 'days');
};
...
const isRefreshToken = (req) => {
  return req.method === 'POST' && req.path === '/user/refresh-token';
};
...
const generateJwt = async (wrestler, user) => {
  const tokenTimeout = _.get(wrestler, 'options.jwtTimeout', '1h');
  const jwtSign = util.promisify(jwt.sign);
  return await jwtSign(transformOne(user), JWT_SECRET_KEY, { algorithm: 'HS512', expiresIn: tokenTimeout });
};
